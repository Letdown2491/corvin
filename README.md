...
- Full wallets — single-signature HD, **multisig**, **Silent Payments** (BIP-352),
  and watch-only — from a seed, an xpub/ypub/zpub, a descriptor, or a hardware
  wallet
...
- **Hardware wallets** — BitBox02, Ledger, Trezor (USB), plus QR / air-gapped
  PSBT signing
- Connects to **your own** Electrum server (Fulcrum, electrs, ElectrumX, …) or
...
- **Rust** + [BDK 3](https://bitcoindevkit.org/) (`bdk_wallet 3`,
  `bdk_electrum 0.23`, `bitcoin 0.32`) — Bitcoin logic, Electrum sync
...
just bundle-desktop   # installers (.deb/.rpm/.AppImage / .dmg / .msi)
```
...
- [ ] Lightning support via NWC (Nostr Wallet Connect, NIP-47)
- [ ] Native mobile apps via Tauri 2 (post-1.0, Android-first; QR/PSBT signing rather
  than USB hardware wallets — the PWA already covers mobile in the browser)
