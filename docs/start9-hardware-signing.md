...
- **StartOS can't do it.** The package manifest exposes a Docker image, named volume
  mounts, network ports, health checks, and backups. It exposes **no USB device
  passthrough, no privileged mode, no host-device mounting** (`/dev/bus/usb`,
  `/dev/hidraw`). A package physically cannot reach a USB device plugged into the
  server. (StartOS manifest spec.)
...
- The transports are fragmented: Ledger/BitBox use WebHID/WebUSB, Trezor uses Trezor
  Connect (iframe), and **Jade uses WebSerial with no browser JS library** (only a
  Python client). It can't cleanly do Jade or Coldcard at all.
...
- Backend `combinePsbt` merges and reports `ready`/`sigs_present`/`signed_fingerprints`,
...
| Ledger / Trezor / BitBox02 | no QR, so a PSBT file/paste shuffle via their vendor app | ⚠️ works, clunkier; WebHID later |

## Build impact (ties to #19)
Because Start9 signs via QR/PSBT, the Start9 build compiles with the **`hw` cargo
feature OFF**:
- Drops `bitbox-api` + `ledger-transport-hid` (native USB), for a smaller binary and
  less attack surface.
...
- **Jade labeling: done.** Named in `QrSignFlow` (comment + UR format tooltip) and the
  ...
  `crypto-psbt` path by format; it needs confirming end-to-end on the device).
- **No-camera fallback: done.** `QrSignFlow` now has a "No camera? Import the signed
  ...
  fee-bump alike. Help article updated.
