...
A structured guide for handing Corvin (desktop, macOS / Windows) to outside
testers. The goal is **not** to verify that features technically work — it's to
find out whether someone who *isn't the developer* finds the wallet clear,
trustworthy, and pleasant to use. Functional/security verification lives in
`verification-plan.md`; this is about **usability**.

Most of this is fill-in-the-blank. Testers grade tasks as they go and answer a
few questions at the end. The organizer collects the filled-in copies.

---

## Part 0 — For the organizer (fill this in before sending)

Replace the bracketed bits, delete this box from the tester's copy if you like.
...
- **Don't help** unless they're truly stuck (give it ~60–90s). Note *where* and *why* they got stuck; that's the finding.
...
**Install warnings ≠ app UX.** If the build isn't notarized (macOS) / signed (Windows), testers will hit "*can't be opened, Apple cannot check it*" or a SmartScreen "*unrecognized app*" prompt. That's a real trust hit, but it's about your distribution, not Corvin's design — tell testers how to get past it (and that it's expected for a test build) so it doesn't skew the trust/first-impression answers. Note separately whether the install step itself scared anyone.

---

## Part 1 — For the tester: how to use this guide

You're helping make sure Corvin is good enough for normal people to use. There
are no wrong answers and **nothing here is a test of you** — if something is
confusing, that's a bug in the app, not in you. Be blunt; polite feedback is
useless feedback.
...
Do these in order. Each has a **task**, then questions. Don't read ahead — react
to what's in front of you.

### Scenario 1 — First impressions (don't click anything yet)

Open the app and look at the first screen for ~15 seconds before doing anything.
...
Task: **Get an address someone could send Bitcoin to.** Imagine you're going to
...
Task: **Find your transaction history and open the details of one transaction.**
(If the wallet has none yet, the organizer will point you at one that does.)
...
Task: **Send a small amount to an address the organizer gives you.** Go through
to the point where you'd normally confirm/sign — follow the prompts.
...
Task: **Add a second wallet from a recovery phrase or an xpub** the organizer
gives you (a watch-only one is fine).
...
Task (no clicking guidance): find each of these. Tick if you found it in under ~20s.
...
Task: **Open the Backend settings and the Display settings and change something**
(e.g. toggle a display option, look at the server settings). You don't have to
...
The wallet ships a set of extra tools. Some live on the **Tools tab**; a few live
in the **⋯ (more) menu** at the top of the wallet. This scenario is about whether
people can find them and understand what they're for — you don't need to be an
...
**9b — Actually use a couple.** Try at least **two** of these. The organizer will
hand you any inputs you need (a sample to paste, etc.).
...
> **Scoring (organizer):** odd items score `(answer − 1)`; even items score `(5 − answer)`. Sum all ten, multiply by 2.5 → a 0–100 score. Rough read: **>80 excellent, 68 ≈ average, <50 needs work.**
...
- Group **Issue log** entries by screen; count how many testers hit the same thing (repeat hits = real problems, not one-offs).
