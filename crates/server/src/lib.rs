...
pub(crate) async fn run_startup_after_unlock(app_state: &state::AppState) -> anyhow::Result<()> {
    ...
    *app_state.config.write().await = cfg;
    // Annotations (labels, categories, frozen UTXOs, cost basis) and the price cache
    // loaded empty in AppState::new because the vault was still locked then. Re-read them
    // now that it's unlocked, before any save can overwrite the sealed files with empty maps.
    app_state.annotations.reload().await;
    app_state.prices.reload().await;
