...
- Strategy / competitive positioning vs Sparrow + candidate directions: kept in private
  notes outside the repo (`../corvin-positioning.md`), not published.
- Verification passes: #89 Taproot HW roundtrip, #90 Ledger multisig HMAC, plus
  the full hands-on pass in the private verification plan (`../corvin-verification-plan.md`,
  not in the repo): timelock gate, HW policy signing, SP spend, BIP-353, payjoin.
- Review passes (security, bug/perf, privacy, robustness) are consolidated in
  `docs/audits.md`. All findings shipped: whole-wallet-consolidation fix, SP-scanner
  lifecycle, configurable BIP-353 DoH resolver (`backend.bip353_doh_url`),
  change-script-mismatch + SP-send input-side warnings (`send.rs`), and at-rest
  encryption (see the feature section above).
...
- **Env *seed* vars (config-value seeds, not hard overrides — see `config.rs`):**
  `CORVIN_ELECTRUM_URL` seeds the default Electrum backend on first run only
  (ignored once `config.toml` exists). `CORVIN_MEMPOOL_URL` seeds the mempool base
  URL (no `/api` suffix) and re-applies on *every* boot while the value is still the
  `mempool.space` default — so a self-hosted mempool added after install is picked
  up — but never clobbers a URL the user changed. Start9 uses both (Fulcrum +
  optional Mempool).
...
- **Audit records:** `docs/audits.md` is a consolidated summary of the security,
  bug/perf, privacy, and robustness review passes (what was checked, found, and
  fixed). Add a new pass there when you do one.
